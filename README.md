...
```text
        ...
        └────┬─────┘  └────┬─────┘ └───┬───┘ └───┬───┘ └────┬─────┘ └───┬───┘ └────┬─────┘
             ...
             └─────────────┴───────────┴────┬────┴──────────┴───────────┴──────────┘
                          ...
                          │   Protocols (base.py) · Gemini / OpenAI-compat /   │
...
| **Anthropic** | `ANTHROPIC_API_KEY` | Generation only — no embeddings API, and browser-use ships no Anthropic client, so those roles fall back to Gemini. |
...
backend/
  ...
  llm/         Provider-agnostic LLM layer: base Protocols, factory, Gemini/OpenAI-compat/Anthropic adapters
...
- **AI:** provider-agnostic via `backend/llm/` — Google Gemini, any OpenAI-compatible endpoint (incl. DeepSeek, Ollama, LM Studio, vLLM), and Anthropic; browser automation via `browser-use` + Playwright/Chromium.
